...
**Scope note (updated for iteration 3):** the pluggable disguise abstraction shipped in **iteration 3**, not iteration 2. The calculator is now one of three lock faces, and this document is the design authority for **the calculator face specifically** — its layout, engine semantics, copy and motion are unchanged by that refactor and must stay byte-identical. What generalized to all faces is recorded in `docs/plans/iteration-3-decisions.md`: the semantic caption kinds (§1.3), the shake parameters (§2.2), the caption-strip metrics, and the reset-to-resting-face-on-lock guarantee (§1.5). Three statements in this document are narrowed to the calculator by that spec and are annotated inline: the VerifyCurrent-only shake (§5.6), the zero-strings-when-locked rule (§6), and the "press =" phrasing of caption copy.
...
- **Shake:** the display readout (not the whole screen) translates horizontally **±8pt, 3 cycles, 300ms total**, ease-in-out, on any failed VerifyCurrent commit. Respect the platform reduce-motion setting: with reduce-motion on, skip the shake and rely on the caption change alone.
  **Iteration 3** reuses these exact parameters for the two overt faces, and there they also fire on the **lock screen** — a wrong PIN or pattern shakes and clears, where the calculator stays silent. The calculator's own behavior is unchanged: it shakes in VerifyCurrent only, and never while locked (`iteration-3-decisions.md` §1.1, §2.2).
...
**Iteration-3 scope note.** Everything in this table is now the **calculator face's** rendering of the host's semantic caption kinds (`iteration-3-decisions.md` §1.3), and the calculator must render these strings verbatim for each kind — that is an acceptance check of the refactor. The two overt faces added in iteration 3 author their own genre-styled equivalents, keyed by the same semantic kinds, and each names its own commit gesture rather than "press =". The zero-strings-when-locked rule above is likewise calculator-specific: the PIN pad and pattern faces render a static face title ("Enter PIN", "Draw your pattern") in the caption slot while locked, since an overt lock screen is not pretending to be anything else.
